    ...
    for other_blocks in range(3):
        ...
        if len(key_creation) == 4:
            for symbol in range(len(key_creation)):
                ...
                if key_creation[symbol] in DOUBLE_NUMBERS:
                    key_creation[symbol] = DOUBLE_NUMBERS[DOUBLE_NUMBERS.index(key_creation[symbol])+int(a[0])] # Второй блок ключа
        ...
        if len(key_creation) == 3:
            for symbol in range(len(key_creation)):
                ...
                if key_creation[symbol] in DOUBLE_NUMBERS:
                    key_creation[symbol] = DOUBLE_NUMBERS[DOUBLE_NUMBERS.index(key_creation[symbol])-int(a[1])] # Третий блок ключа
        ...
        if len(key_creation) == 2:
            for symbol in range(len(key_creation)):
                ...
                if key_creation[symbol] in DOUBLE_NUMBERS:
                    key_creation[symbol] = DOUBLE_NUMBERS[DOUBLE_NUMBERS.index(key_creation[symbol])+int(a[2])] # Четвёртый блок ключа
...
root = tk.Tk()
root.title('Генератора ключа')          # Запуск программы и размеры
root.geometry('1300x800')



bg_image = tk.PhotoImage(file='Crypto.png')
label_bg = tk.Label(root, image=bg_image)            # Фон
